...
public class TimePointTest extends TestCase {
	...
	public void testCreationWithDefaultTimeZone() {
		...
		assertEquals("pm hours in 12hr clock", TimePoint.atGMT(2004, 1, 1, 12, 0),
				TimePoint.at12hr(2004, 1, 1, 12, PM, 0, 0, 0, gmt));
	}
	...
	public void testEqualsOverYearMonthDay() {
		...
		assertTrue("midnight (in the moring), start of same day",
				TimePoint.atMidnightGMT(2000, 1, 1).isSameDayAs(thePoint, gmt));
		
		assertFalse("midnight (night), start of next day",
				TimePoint.atMidnightGMT(2000, 1, 2).isSameDayAs(thePoint, gmt));
