 ...
 * ({@link jp.tricreo.basicunits.time.CalendarDate})をはじめ、「特定の瞬間を表す不変(immutable)クラス」
 * ({@link jp.tricreo.basicunits.time.TimePoint})、「期間（時間の範囲）を表すクラス」
 * ({@link jp.tricreo.basicunits.time.CalendarInterval}や{@link jp.tricreo.basicunits.time.TimeInterval})、
 * 「時間量を表すクラス」({@link jp.tricreo.basicunits.time.Duration})等の基本クラスを提供する。</p>
 */
