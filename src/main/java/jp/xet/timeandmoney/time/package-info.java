 ...
 * ({@link jp.xet.timeandmoney.time.CalendarDate})をはじめ、「特定の瞬間を表す不変(immutable)クラス」
 * ({@link jp.xet.timeandmoney.time.TimePoint})、「期間（時間の範囲）を表すクラス」
 * ({@link jp.xet.timeandmoney.time.CalendarInterval}や{@link jp.xet.timeandmoney.time.TimeInterval})、
 * 「時間量を表すクラス」({@link jp.xet.timeandmoney.time.Duration})等の基本クラスを提供する。</p>
 */
